		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.mysql_tables.test_util", "database", "mysql"),
					resource.TestCheckResourceAttr("data.mysql_tables.test_util", "pattern", "%"),
					testAccTablesCount("data.mysql_tables.test_util", "tables.#", func(rn string, table_count int) error {
						if table_count < 1 {
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.mysql_tables.test_util", "database", "mysql"),
					resource.TestCheckResourceAttr("data.mysql_tables.test_util", "pattern", "__table_does_not_exist__"),
					testAccTablesCount("data.mysql_tables.test_util", "tables.#", func(rn string, table_count int) error {
						if table_count > 0 {
